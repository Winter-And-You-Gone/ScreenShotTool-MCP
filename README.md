...
- `launch_app`: 启动指定 `.exe`，可等待第一个可见窗口。
- `list_windows`: 按 `pid`、进程名、标题关键字列出可见窗口。
- `capture_window`: 按 `hwnd` / `pid` / 进程名 / 标题关键字截取窗口，可传窗口内相对矩形，也可用 `focus:false` 保留弹出菜单/浮层。
...
- `close_app`: 用 `taskkill` 关闭指定 `pid`。
- `type_text`: 聚焦窗口后输入常用 ASCII 文本。
...
- 键盘输入面向常用 ASCII 文本和常见功能键，不覆盖 IME/Unicode 全量输入。
- `exePath` 要求绝对 `.exe` 路径。
