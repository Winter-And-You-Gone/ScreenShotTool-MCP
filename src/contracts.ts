...
export const contracts: Record<string, ToolContract> = {
  ...
  capture_window: {
    name: "capture_window",
    description: "Captures the visual contents of a target window as an image file. Use when the task requires visual content, layout inspection, rendering verification, or an image artifact - including when the user explicitly asks for a screenshot. Use the targetRef as the authoritative target session: do NOT call list_windows merely because a capture backend fails or returns CAPTURE_GEOMETRY_MISMATCH - retry against the same targetRef with an appropriate capture method. 'print' mode uses PrintWindow (works occluded/minimized, misses separate popup/tooltip windows); 'screen' mode copies the visible screen. IMPORTANT: interactionMode=background intentionally forces the non-activating PrintWindow path, so captureMethod='screen' alone does NOT bypass background mode. To recover from a PrintWindow-specific blank/geometry failure using visible-screen capture, explicitly request BOTH captureMethod='screen' AND interactionMode='foregroundDemo' against the same targetRef (never only captureMethod). The result reports the ACTUAL backend that executed (interaction.method / captureBackend), which may differ from the requested method when background mode overrides it. Capture failures should be described by observable evidence (blank frame, geometry mismatch, target disappearance, etc.) - do not infer toolkit-specific root causes without diagnostic evidence. OMIT outputPath unless the user specified a target file - the server writes to its default writable outputs/ directory and returns the path (never invent a path like X:\\...). Returns: path,width,height,target,rect,timestamp,interaction,geometry. Pipe-safe: path, interaction.",
